...
<%--        <h1><fmt:message key="new_account"/></h1><br/>--%>

<%--        <form method="post" action="${pageContext.request.contextPath}/wallet/new_account">--%>

<%--            <input class="mt-3" type="text" name="last_name" value="i<c:if test="${last_name!=null}">${last_name}</c:if>" required>--%>
...
<%--&lt;%&ndash;                    "<c:if test="${email!=null}">${email}</c:if>" required&ndash;%&gt;--%>
<%--            >--%>
