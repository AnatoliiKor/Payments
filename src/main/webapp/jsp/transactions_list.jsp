            ...
            <table class="table table-striped auto__table table-condensed text-center">
                ...
                <c:forEach var="transaction" items="${transactions}"
                    ...
                    <tr>
                        ...
                        <td
                                <c:if test="${sort_by.equals('amount')}">class="text-info fw-bold"</c:if>>${account_type.equals('payer')? '-' : ''}${transaction.amount/100}</td>
                        <td
                                <c:if test="${sort_by.equals('currency')}">class="text-info fw-bold"</c:if>>${transaction.currency}</td>
                        <td <c:if test="${sort_by.equals('receiver')}">class="text-info fw-bold"</c:if>>
                            UA${transaction.receiver}</td>
                        <td
