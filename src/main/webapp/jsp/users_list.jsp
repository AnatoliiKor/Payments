...
<div class="container mt-2">
    ...
    <ul class="pagination">
        <li class="page-item ${page == 1 ? 'disabled' : ''}"><a class="page-link" href="/users?page=${page-1}"><fmt:message
                key="previous_main"/></a></li>
        <li class="page-item active"><a class="page-link">${page}</a></li>
        <li class="page-item ${page == pg_max? 'disabled' : ''}"><a class="page-link"
                                                                  href="/users?page=${page+1}"><fmt:message
                key="next_main"/></a></li>
    </ul>
    ...
    <table class="table table-striped auto__table table-condensed text-center">
        ...
        <c:choose>
            <c:when test="${not empty users}">
                <c:forEach var="user" items="${users}" varStatus="position" begin="${10 * (page - 1)}"
                           end="${10 * (page - 1) + 9}">
                    <tr>
                        ...
                        <td><a href="${pageContext.request.contextPath}/wallet/user?id=${user.id}"><fmt:message
                                key="profile_nav"/></a></td>
                        <td><a href="${pageContext.request.contextPath}/wallet/accounts?user_id=${user.id}"><fmt:message
                                key="accounts"/></a></td>
                    </tr>
                </c:forEach>
            </c:when>
