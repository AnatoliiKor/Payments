        ...
        <tbody>
                <c:forEach var="payment" items="${payments}"
                    ...
                    <tr >
                        ...
                        <td <c:if test="${payment_sort_by.equals('account_number')}">class="text-info fw-bold"</c:if>>UA${payment.accountNumber}</td>
                        <td <c:if test="${payment_sort_by.equals('account_name')}">class="text-info fw-bold"</c:if>>${payment.accountName}</td>
                        <td <c:if test="${payment_sort_by.equals('registered')}">class="text-info fw-bold"</c:if>>${payment.getFormatedDate()}</td>
                        <td>${payment.destination}</td>
                        <td <c:if test="${payment_sort_by.equals('amount')}">class="text-info fw-bold"</c:if>>${payment.amount/100}</td>
                        <td <c:if test="${payment_sort_by.equals('currency')}">class="text-info fw-bold"</c:if>>${payment.currency}</td>
                        <td <c:if test="${payment_sort_by.equals('receiver')}">class="text-info fw-bold"</c:if>>${payment.receiver}</td>
                    </tr>
                </c:forEach>
        </tbody>
    </table>
